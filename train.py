...
class ImageSequence(keras.utils.Sequence):
    def __init__(self, directory_name, directory_csv, batch_size, captcha_length, captcha_symbols, captcha_width, captcha_height):
        self.directory_name = directory_name
        self.directory_csv = directory_csv
        ...
        # read image content from csv file
        with open(directory_csv, 'r', encoding="utf-8") as csvfile:
            reader = csv.reader(csvfile)
            rows = [row for row in reader]
        # image content: image file name
        self.files = dict(zip(map(lambda x: x[0], rows), map(lambda x: x[1], rows)))
        self.used_files = []
    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.batch_size):
            ...
            raw_data = cv2.imread(os.path.join(self.directory_name, random_image_label))
            ...
            for j, ch in enumerate(random_image_file):
                y[j][i, :] = 0
...
def main():
    ...
    parser.add_argument('--train-csv', help='Where to look for the training image csv', type=str)
    parser.add_argument('--validate-dataset', help='Where to look for the validation image dataset', type=str)
    parser.add_argument('--validate-csv', help='Where to look for the validation image csv', type=str)
    ...
    if args.train_csv is None:
        print("Please specify the csv filename list of training data set")
        exit(1)
    ...
    if args.validate_csv is None:
        print("Please specify the csv filename list of validation data set")
        exit(1)
    ...
    # with tf.device('/device:XLA_CPU:0'):
        ...
        training_data = ImageSequence(args.train_dataset, args.train_csv, args.batch_size, args.length, captcha_symbols, args.width, args.height)
        validation_data = ImageSequence(args.validate_dataset, args.validate_csv, args.batch_size, args.length, captcha_symbols, args.width, args.height)
